...
LUA_API int errorfunc(lua_State *L) {
	...
	lua_pushnumber(L, 2);
	lua_call(L, 2, 1);
	int ret = lua_getglobal(L, "CrashTrack");
	if(ret != 0){
...
//wjfadd//
LUA_API int xlua_getuservalue(lua_State *L) {
#if LUA_VERSION_NUM > 501
    lua_getuservalue(L, 1);
#else
    lua_getfenv(L,1);
...
//wjfadd//
//1.obj  2.table
LUA_API int xlua_setuservalue(lua_State *L) {
#if LUA_VERSION_NUM > 501
...
static const luaL_Reg xlualib[] = {
    ...
    {"getuservalue", xlua_getuservalue},
    {"setuservalue", xlua_setuservalue},
    {NULL, NULL}
};
